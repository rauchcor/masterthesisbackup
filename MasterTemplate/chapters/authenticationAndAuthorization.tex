...
A digital identity as explained above is the result of what is called the authentication process. It is a way of identifying the user as whom he claims to be. In order to proof the users identity presentation, validation and verification of a minimum set of attributes is necessary [cf. (\citet{Boyed:2012:GSOA})]. 
...
OAuth 2.0 is a protocol that is used for access control. The protocol adds an authori-zation layer – separating the Client from the Resource Owner. To gain access, an access token is used instead of the resource owner’s credentials. Access Tokens are issued by an Authorization Server and can be used  by the Client to gain access to protected resources hosted by the resource server. OAuth 2.0 uses a token service, which centralizes this logic and reduces complexity for the Client and the APIs [cf. (\cite{Brock:2018:ID4})]
 ...
 \begin{enumerate}
 	\item \textbf{Authorization Code Flow}  - The Authorization Code Flow returns an Authentification Code which can be used to be exchanged for an id\_token or an access\_token. This is used for Clients that can maintain a secret securly between themselves and the Authorization Server. Typically this aproach is used for sever-to-server communication when no interactive user is present. In this thesis, I focus on  user interaction thus, Ido not include an example [cf. (\cite{Sakimura:2014:OpenIDConnect}, p.10)]
